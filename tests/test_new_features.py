...
def test_observation_dimensions():
    ...
    # With opponent tracking: 53 base + MAX_OPPONENTS × FEATURES_PER_OPPONENT
    env = TexasHoldemEnv(num_players=3, track_opponents=True)
    obs, _ = env.reset()
    expected_with = 53 + env.MAX_OPPONENTS * env.FEATURES_PER_OPPONENT
    assert obs.shape == (expected_with,), f"Expected ({expected_with},), got {obs.shape}"
    assert obs.shape == env.observation_space.shape
    print(f"✓ With tracking: {obs.shape}")

    # Without opponent tracking: just the 53 base dims.
